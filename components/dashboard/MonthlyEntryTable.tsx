...
function MonthlyRow({ monthIndex, caAmount, cumulativeCA, onSave, activityType, acreEnabled }: MonthlyRowProps) {
    ...
    return (
        <tr className={`border-b border-slate-100 ${rowBgClass}`}>
            <td className={`p-4 text-[13px] font-medium ${hasData ? 'text-slate-900' : 'text-slate-500'}`}>
                {MONTHS[monthIndex]}
            </td>
            <td className="p-4">
            ...
            <td className={`p-4 text-right text-[13px] font-medium ${hasData && tva.status !== 'safe' ? 'text-amber-600' : hasData ? 'text-slate-600' : 'text-slate-400'}`}>
                {hasData ? new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(cumulativeCA) : '\u2013'}
            </td>
            <td className="p-4 text-right text-[13px] text-slate-400">
...
export function MonthlyEntryTable() {
    ...
    if (loading) {
        return (
            <div className="flex min-h-[400px] items-center justify-center rounded-[12px] border border-slate-200 bg-white">
    ...
    const tva = getTVAStatus(totalCA, config.activity_type);

    return (
        <div className="relative flex flex-col overflow-hidden rounded-[12px] border border-slate-200 bg-white">
            {isCompletelyEmpty && (
                <div className="pointer-events-none absolute left-1/2 top-[140px] z-10 flex -translate-x-1/2 animate-bounce items-center gap-3 whitespace-nowrap rounded-xl bg-slate-900 px-5 py-3 text-[13px] font-medium text-white shadow-xl">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 p-6">
                <div>
                    <h2 className="text-lg font-medium text-slate-900">Chiffre d'Affaires Mensuel</h2>
                    <p className="mt-1 text-[12px] text-slate-500">Saisissez vos encaissements HT.</p>
                </div>
            </div>
            ...
            <div className="overflow-hidden">
                {tva.status !== 'safe' && (
                    <div className="px-6 pb-2">
                        <div className="tva-alert-banner mb-2 flex items-center gap-2 rounded-[8px] border border-[#f09595] bg-[#fcebeb] px-3 py-2 text-[12px] text-[#791f1f]">
                            <span className="h-2 w-2 rounded-full bg-[#e24b4a]" />
                            <span className="flex-1">
                                Seuil TVA franchi - {tva.status === 'danger' ? `${((totalCA / tva.threshold) * 100).toFixed(0)}%` : 'bientôt atteint'} - Préparez votre bascule
                            </span>
                            <a href="/dashboard/optimisation" className="font-medium text-[#0c447c]">
                                Voir suivi -&gt;
                            </a>
                        </div>
                    </div>
                ...
                <div className="max-h-[520px] overflow-auto xl:max-h-[430px]">
                    <table className="w-full border-collapse text-left">
                        <thead>
                            <tr className="sticky top-0 z-[1] border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-[0.04em] text-slate-500">
                        ...
                        <tfoot className="bg-[#0d1b35] text-white">
                            <tr>
                                <td className="rounded-bl-3xl p-4 text-[13px] font-medium">Total Annuel</td>
                                <td className="p-4 text-right text-lg font-medium">
                                    {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(runningTotal)}
                                </td>
                                <td className="p-4"></td>
                                <td className="rounded-br-3xl p-4 text-right text-[14px] font-medium text-red-300">
