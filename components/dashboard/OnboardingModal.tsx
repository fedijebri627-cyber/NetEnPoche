...
export function OnboardingModal() {
    ...
    return createPortal(
        <>
            <div className="fixed inset-0 z-50 bg-slate-950/65 backdrop-blur-sm" />
            <div className="fixed inset-0 z-[51] overflow-y-auto p-3">
                <div className="mx-auto my-2 flex max-h-[calc(100dvh-16px)] w-full max-w-[620px] flex-col overflow-hidden rounded-[18px] bg-white shadow-2xl">
                <div className="shrink-0 border-b border-slate-100 bg-[radial-gradient(circle_at_top_left,_rgba(0,200,117,0.2),_transparent_35%),linear-gradient(135deg,#0d1b35_0%,#13264a_100%)] px-3 py-2.5 text-white sm:px-4 sm:py-3">
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            <div className="mb-1.5 inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-100">
                                <Sparkles className="h-3 w-3" />
                                Onboarding guidé
                            </div>
                            <h2 className="font-syne text-lg font-bold leading-tight sm:text-[1.35rem]">Configurez NetEnPoche en 4 étapes</h2>
                        ...
                        <button
                            ...
                            className="rounded-full border border-white/10 bg-white/10 p-1 text-slate-100 transition hover:bg-white/15"
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-4">
                        {['Activité', 'Fiscalité', 'Objectif', 'Synthèse'].map((label, index) => {
                            ...
                            return (
                                <div
                                    key={label}
                                    className={`rounded-md border px-2 py-1 text-[10px] transition ${active ? 'border-emerald-300 bg-emerald-400/20 text-white' : completed ? 'border-white/10 bg-white/10 text-slate-100' : 'border-white/10 bg-white/5 text-slate-300'}`}
                                >
                                    <div className="text-[9px] font-semibold uppercase tracking-wide">Étape {index + 1}</div>
                                    <div className="mt-0.5 font-semibold leading-tight">{label}</div>
                                </div>
                            );
                        })}
                    </div>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2.5 sm:px-3.5 sm:py-3">
                    {step === 1 && (
                        <div className="space-y-3">
                            <div>
                                <h3 className="font-syne text-sm font-bold text-[#0d1b35] sm:text-base">Quel est votre type d’activité principal ?</h3>
                                <p className="mt-1 text-[11px] leading-4.5 text-slate-500">Cette réponse pilote les taux URSSAF, la TVA et les comparaisons fiscales.</p>
                            </div>
                            <div className="grid gap-2.5 sm:grid-cols-2">
                                {activityOptions.map((option) => (
                                    <button
                                        ...
                                        className={`rounded-xl border p-2.5 text-left transition ${activityType === option.id ? 'border-emerald-400 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <div className="font-syne text-[14px] font-bold text-[#0d1b35]">{option.label}</div>
                                                <p className="mt-1 text-[11px] leading-4.5 text-slate-500">{option.description}</p>
                                            </div>
                    ...
                    {step === 2 && (
                        <div className="space-y-3">
                            <div>
                                <h3 className="font-syne text-sm font-bold text-[#0d1b35] sm:text-base">Quel est votre cadre fiscal actuel ?</h3>
                                <p className="mt-1 text-[11px] leading-4.5 text-slate-500">On affine vos estimations d’impôt et vos recommandations.</p>
                            </div>

                            <div className="grid gap-2.5 lg:grid-cols-2">
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    <div className="text-[12px] font-semibold text-slate-700">Situation familiale</div>
                                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                                        {householdOptions.map((option) => (
                                            <button
                                                ...
                                                className={`rounded-lg border px-2 py-1.5 text-[10px] font-semibold transition sm:text-[11px] ${situationFamiliale === option.value ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}
                                            >
                                                {option.label}
                                            </button>
                                        ))}
                                ...
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    <div className="grid gap-2.5 sm:grid-cols-2">
                                        <label className="space-y-1.5 text-[12px] font-semibold text-slate-700">
                                            ...
                                            <input
                                                ...
                                                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-medium text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 sm:text-xs"
                                            />
                                        </label>
                                        <label className="space-y-1.5 text-[12px] font-semibold text-slate-700">
                            ...
                            <div className="grid gap-2.5 lg:grid-cols-2">
                                <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    <input
                                        type="checkbox"
                                        checked={acreEnabled}
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-0.5 text-[11px] leading-4 text-slate-500">Réduction temporaire du taux de charges sociales en début d’activité.</div>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-0.5 text-[11px] leading-4 text-slate-500">L’impôt est prélevé avec vos cotisations à un taux fixe sur le CA.</div>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    {step === 3 && (
                        <div className="space-y-3">
                            <div>
                                <h3 className="font-syne text-sm font-bold text-[#0d1b35] sm:text-base">Quel niveau d’activité visez-vous ?</h3>
                                <p className="mt-1 text-[11px] leading-4.5 text-slate-500">Ces deux champs permettent de démarrer avec des objectifs et des alertes utiles.</p>
                            </div>

                            <div className="grid gap-2.5 lg:grid-cols-2">
                                <label className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    <span className="text-[12px] font-semibold text-slate-700">Objectif de CA annuel</span>
                                    <div className="relative mt-2.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-lg border border-slate-200 bg-white py-1.5 pl-8 pr-3 text-[11px] font-semibold text-slate-800 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 sm:text-xs"
                                        />
                                    </div>
                                    <p className="mt-1.5 text-[11px] leading-4.5 text-slate-500">Optionnel, mais très utile pour activer la lecture de votre avance ou retard.</p>
                                </label>

                                <label className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                                    <span className="text-[12px] font-semibold text-slate-700">CA encaissé ce mois-ci</span>
                                    <div className="relative mt-2.5">
                                        <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-base font-bold text-slate-400">€</span>
                                        <input
                                            type="number"
                                    ...
                                    <p className="mt-1.5 text-[11px] leading-4.5 text-slate-500">Si vous le renseignez maintenant, les cartes de réserve et de projection seront déjà actives.</p>
                                </label>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 4 && (
                        <div className="space-y-3">
                            <div>
                                <h3 className="font-syne text-sm font-bold text-[#0d1b35] sm:text-base">Votre cockpit de départ est prêt</h3>
                                <p className="mt-1 text-[11px] leading-4.5 text-slate-500">Voici les repères qui seront activés dès la fermeture de l’onboarding.</p>
                            </div>
                            ...
                            <div className="grid gap-2.5 md:grid-cols-[1.08fr_0.92fr]">
                                <div className="rounded-[18px] border border-slate-200 bg-white p-3 shadow-sm">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            <div className="font-syne text-[14px] font-bold text-[#0d1b35]">Résumé de configuration</div>
                                            <p className="mt-1 text-[11px] leading-4.5 text-slate-500">Activité, foyer fiscal et objectif utilisés dans le dashboard.</p>
                                        </div>
                                        <div className="rounded-lg bg-emerald-50 px-2 py-1.5 text-right">
                                            <div className="text-[10px] font-semibold uppercase tracking-wide text-emerald-600">Net estimé du mois</div>
                                            <div className="font-syne text-[15px] font-black text-emerald-600 sm:text-base">{formatCurrency(previewBreakdown.netReel)}</div>
                                        </div>
                                    </div>

                                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                                        <div className="rounded-lg border border-slate-100 bg-slate-50 p-2 text-[10px] text-slate-600 sm:text-[11px]">
                                ...
                                <div className="rounded-[18px] border border-emerald-100 bg-emerald-50 p-3 shadow-sm">
                                    <div className="flex items-center gap-3 text-emerald-700">
                                        <div className="rounded-lg bg-white p-1.5 shadow-sm">
                                            <PiggyBank className="h-4 w-4" />
                                        </div>
                                        <div>
                                            <div className="font-syne text-[14px] font-bold">{previewActions.length} actions activées</div>
                                            <p className="mt-0.5 text-[10px] leading-4 text-emerald-700/80">Ces priorités seront visibles dès l'arrivée sur le dashboard.</p>
                                        </div>
                                    </div>

                                    <ul className="mt-2.5 space-y-1.5">
                                        {previewActions.map((action) => (
                                            <li key={action} className="rounded-lg border border-emerald-100 bg-white/80 px-2 py-1.5 text-[10px] leading-4 text-slate-700">
                                                {action}
                                            </li>
                                        ))}
                                    </ul>
                ...
                <div className="shrink-0 flex flex-col gap-2 border-t border-slate-100 bg-slate-50 px-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-4">
                    <div className="flex items-center gap-3">
                        {step > 1 ? (
                            <button
                                type="button"
                                onClick={() => setStep((current) => Math.max(1, current - 1))}
                                className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 transition hover:bg-slate-100 sm:text-xs"
                            >
                                Retour
                            </button>
                        ) : (
                            <button
                                type="button"
                                onClick={() => closeModal(true)}
                                className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-500 transition hover:bg-slate-100 sm:text-xs"
                            >
                                Plus tard
                            </button>
                        )}
                    ...
                    {step < 4 ? (
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#0d1b35] px-3.5 py-1.5 text-[11px] font-bold text-white transition hover:bg-[#13264a] sm:text-xs"
                        >
                            Continuer
                        ...
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-500 px-3.5 py-1.5 text-[11px] font-bold text-white transition hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-70 sm:text-xs"
                        >
