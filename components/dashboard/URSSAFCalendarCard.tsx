...
export function URSSAFCalendarCard() {
    ...
    const StatusBadge = ({ status, hasData }: { status: string; hasData: boolean }) => {
        if (!hasData && status !== 'paid') {
            return <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-medium uppercase text-slate-400">CA manquant</span>;
        }
        if (status === 'paid') return <span className="flex items-center gap-1 rounded bg-[#00c875]/10 px-2 py-0.5 text-[10px] font-medium uppercase text-[#00c875]"><CheckCircle2 className="h-3 w-3" /> Pay?</span>;
        if (status === 'upcoming') return <span className="flex items-center gap-1 rounded bg-[#f5a623]/10 px-2 py-0.5 text-[10px] font-medium uppercase text-[#f5a623]"><Clock className="h-3 w-3" /> ? venir</span>;
        return <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-medium uppercase text-slate-400">Estim?</span>;
    };
        ...
        <div className="flex h-full min-h-[460px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="relative bg-gradient-to-br from-[#0d1b35] to-[#162848] p-6 text-white">
                ...
                <div className="relative z-10 mb-4 flex items-start justify-between gap-4">
                    <div>
                        <h3 className="mb-1 flex items-center gap-2 text-[12px] font-medium text-slate-300">
                            <CalendarDays className="h-4 w-4" /> Prochaine ?ch?ance
                        </h3>
                        <p className="text-[24px] font-medium tracking-tight">{nextPayment.hasData ? formatCurrency(nextPayment.amount) : '? compl?ter'}</p>
                    </div>
                    <div className="shrink-0 rounded-xl border border-white/10 bg-white/10 p-3 text-center backdrop-blur-md">
                        <span className="mb-1 block text-[22px] font-medium leading-none text-[#00c875]">{daysLeft}</span>
                        <span className="block text-[11px] font-medium uppercase tracking-[0.04em] text-slate-300">Jours</span>
                    </div>
                </div>
                <p className="relative z-10 text-sm text-slate-400">
                    Avant le <strong className="font-medium text-white">{nextPayment.dueDate.toLocaleDateString('fr-FR')}</strong>
                </p>
            </div>
                    ...
                    {quarters.map((quarter) => (
                        <li key={quarter.label} className={`flex items-center justify-between rounded-xl border p-3 ${quarter.status === 'upcoming' ? 'border-[#f5a623]/30 bg-white shadow-sm' : 'border-slate-100 bg-transparent'}`}>
                            <div className="flex items-center gap-3">
                                <div className={`flex h-9 w-9 items-center justify-center rounded-lg text-[13px] font-medium ${quarter.status === 'upcoming' ? 'bg-[#f5a623]/10 text-[#f5a623]' : 'bg-slate-100 text-slate-500'}`}>
                                    {quarter.label}
                                </div>
                                <div className="flex flex-col">
                                    <span className={`text-[14px] font-medium ${quarter.hasData ? 'text-slate-800' : 'text-slate-500'}`}>
