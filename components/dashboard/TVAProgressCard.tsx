...
import { getTVAStatus } from '@/lib/calculations';
...
export function TVAProgressCard() {
    ...
    const tva = getTVAStatus(runningCA, config.activity_type);

    // Gradient calculation based on risk
    let barGradient = 'from-[#00c875] to-[#00c875]/80'; // Safe
    if (tva.percentage >= 70 && tva.percentage < 90) barGradient = 'from-[#00c875] via-[#f5a623] to-[#f5a623]'; // Warning approaching
    if (tva.percentage >= 90) barGradient = 'from-[#f5a623] to-[#e84040]'; // Danger

    // Calculate estimated month of breach if pace continues
    const currentMonth = new Date().getMonth() + 1;
    const avgMonthlyCA = currentMonth > 0 ? runningCA / currentMonth : 0;
    let estimatedBreachText = "Vous avez de la marge cette année.";
    ...
    if (avgMonthlyCA > 0 && tva.remaining > 0) {
        ...
        if (currentMonth + monthsLeft <= 12) {
            ...
            estimatedBreachText = `Estimation de dépassement : ${monthName} ${year}`;
        }
    } else if (tva.percentage >= 100) {
        estimatedBreachText = "Seuil franchi. Vous êtes assujetti à la TVA.";
    }
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm relative overflow-hidden">
            ...
            <div className="flex justify-between items-end mt-2 mb-4">
                <span className="text-sm font-medium text-slate-700">
                    {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(runningCA)}
                </span>
                <span className="text-xs text-slate-400 font-medium">
                    Max: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(tva.threshold)}
                </span>
            </div>
            ...
            <div className={`mt-4 p-3 rounded-xl flex items-start gap-3 text-sm ${tva.percentage >= 90 ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-slate-50 text-slate-600 border border-slate-100'}`}>
                ...
                <div>
                    <p className="font-medium mb-0.5">Marge restante : {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(tva.remaining)}</p>
